...
def _make_projector(im_res, cout, proj_type, checkpoint_path =None,expand=False):
    assert proj_type in [0, 1, 2], "Invalid projection type"

    ### Build pretrained feature network
    model = timm.create_model('tf_efficientnet_lite0',checkpoint_path=checkpoint_path, pretrained=True)
...
class F_RandomProj(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.pretrained, self.scratch = _make_projector(im_res=im_res, cout=self.cout, proj_type=self.proj_type, expand=self.expand,checkpoint_path=checkpoint_path)
        self.CHANNELS = self.scratch.CHANNELS
        self.Pretrain_CHANNELS = self.pretrained.CHANNELS
    ...
    def forward(self, x):
        ...
        if self.proj_type == 0: return features

        out0_channel_mixed = self.scratch.layer0_ccm(features['0'])
        out1_channel_mixed = self.scratch.layer1_ccm(features['1'])
        out2_channel_mixed = self.scratch.layer2_ccm(features['2'])
        out3_channel_mixed = self.scratch.layer3_ccm(features['3'])
