...
def ssim(frames1, frames2):
  ...
  for i in tqdm(range(len(frames1))):
    # error += compare_ssim(frames1[i], frames2[i], multichannel=True, win_size=51) #old
...
def lpips_metric(frames1, frames2):
  error = 0.0
  for i in tqdm(range(len(frames1))):
    error += lpips_2imgs(frames1[i], frames2[i])
  return error/len(frames1)



def lpips_2imgs(fake_img,real_img):
    ...
    # Load images
    img0 = lpips.im2tensor(lpips.load_image(fake_img))  # RGB image from [-1,1]
    img1 = lpips.im2tensor(lpips.load_image(real_img))
...
def calculate_temp_activation_statistics(images, model, batch_size=64,
  dims=2048, cuda=True, verbose=False):
  """Calculation of the statistics used by the FID.
  Params:
  -- images      : Numpy array of dimension (n_images, 3, hi, wi). The values
  ...
  -- verbose     : If set to True and parameter out_step is given, the
                   number of calculated batches is reported.
  Returns:
  -- act    : activations for each samples
  """
  act = get_activations(images, model, batch_size, dims, cuda, verbose)
...
def torch_calculate_frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6):
  ...
  # aaa = (torch.matmul(sigma1,sigma2)).numpy()
  # Product might be almost singular
...
#
#   # Product might be almost singular
#   temp_ = sigma1.dot(sigma2)
#   covmean, _ = linalg.sqrtm(temp_.get(), disp=False)
...
#     temp_ = (sigma1 + offset).dot(sigma2 + offset)
#     covmean = linalg.sqrtm(temp_.get())
#
#     covmean = cp.asarray(covmean)
#     cp.sqrt()
#   # Numerical error might give slight imaginary component
...
#
#   return (diff.dot(diff) + cp.trace(sigma1) + cp.trace(sigma2) - 2 * tr_covmean).get()
