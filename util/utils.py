...
import glob
import torch
from .mask_generator import BatchRandomMask
...
def co_mod_mask(real_image,im_size):

    mask_10 = BatchRandomMask(real_image.shape[0], im_size, hole_range=[0, 1])
    mask_01 = 1 - torch.from_numpy(mask_10).cuda()
...
def linear_norm(tensor):
    # aaa = torch.min(tensor, dim=-1)
    # min_vec = torch.min(torch.min(torch.min(tensor,dim=-1),dim=-1),dim=-1)
    # max_vec = torch.max(torch.max(torch.max(tensor,dim=-1),dim=-1),dim=-1)
    min_vec = tensor.min(dim=-1).values.min(dim=-1).values.min(dim=-1).values.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
    max_vec = tensor.max(dim=-1).values.max(dim=-1).values.max(dim=-1).values.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
    tensor = (tensor - min_vec)/(max_vec-min_vec)
...
def print_networks(net,name, verbose=True):
    ...
    if verbose:
        print(net)
    print('[Network %s] Total number of parameters : %.3f M, including trainable : %.3f M, non trainable : %.3f M'
          % (name, total_num_params / 1e6,trainable_params / 1e6,non_trainable_params / 1e6))
    print('-----------------------------------------------')
