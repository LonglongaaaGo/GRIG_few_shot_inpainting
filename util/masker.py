...
def np_free_form_mask(maxVertex, maxLength, maxBrushWidth, maxAngle, h, w):
    ...
    for i in range(numVertex):
        ...
        brushWidth = np.random.randint(3, maxBrushWidth + 1) // 2 * 2
...
def generate_rect_mask(im_size, mask_size, margin=0, rand_mask=True,dim =3):
    mask = np.zeros((im_size[0], im_size[1])).astype(np.float32)
    if rand_mask:
        sz0, sz1 = mask_size[0], mask_size[1]
        of0 = np.random.randint(margin, im_size[0] - sz0 - margin+1)
        of1 = np.random.randint(margin, im_size[1] - sz1 - margin+1)
    else:
        sz0, sz1 = mask_size[0], mask_size[1]
        of0 = (im_size[0] - sz0) // 2
        of1 = (im_size[1] - sz1) // 2
...
def generate_stroke_mask(im_size, parts=10, maxVertex=20, maxLength=100, maxBrushWidth=24, maxAngle=360,dim = 3):
